...
def add_time(base_dt: datetime, months: int = 0, days: int = 0) -> datetime:
    """
    Добавляет к дате:
      - календарные месяцы (с клэмпингом дня),
      - обычные дни через timedelta.
    ...
    if months != 0:
        ...
        base_dt = base_dt.replace(year=year, month=month, day=clamped_day)

    if days != 0:
        base_dt = base_dt + timedelta(days=days)
