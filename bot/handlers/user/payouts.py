...
from .start import send_own_menu

router = Router(name="user_payouts_router")
    ...
    try:
        if settings.PHOTO_ID_MAIN_MENU:
            await callback.message.edit_media(media=InputMediaPhoto(media=settings.PHOTO_ID_MAIN_MENU, caption=_(key="promo_code_prompt")),
                                              reply_markup=get_back_to_main_menu_markup(current_lang, i18n, callback_data="main_action:own"), disable_web_page_preview=True)
        else:
