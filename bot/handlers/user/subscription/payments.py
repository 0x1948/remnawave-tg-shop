...
async def select_subscription_period_callback_handler(callback: types.CallbackQuery, settings: Settings, i18n_data: dict, session: AsyncSession):
    ...
    logging.info(callback.data.split(":"))
    logging.info(len(callback.data.split(":")))

    try:
        months = int(callback.data.split(":")[1])
        is_gift = True if len(callback.data.split(":")) > 2 else False
        logging.info(f"is_gift: {is_gift}")
    except (ValueError, IndexError):
    ...
    try:
        if settings.PHOTO_ID_PAY_METHOD:
            await callback.message.edit_media(media=InputMediaPhoto(media=settings.PHOTO_ID_PAY_METHOD, caption=text_content), reply_markup=reply_markup, disable_web_page_preview=True)
        else:
