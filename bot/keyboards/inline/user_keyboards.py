...
def get_main_menu_inline_keyboard(
    ...
    if settings.SUPPORT_LINK:
        first_row.append(
            (InlineKeyboardButton(text=_(key="menu_support_button"), callback_data="main_action:help"))
        )
...
def get_help_keyboard(
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text=_(key="menu_help_1_button"), callback_data="help:1")
...
def get_payment_method_keyboard(months: int, price: float,
    ...
    if settings.YOOKASSA_ENABLED:
        builder.button(text=_("pay_with_yookassa_button"),
                       callback_data=f"pay_yk_new:{months}:{price}{add_text}")
...
def get_trial_text_keyboard(lang: str, i18n_instance) -> InlineKeyboardMarkup:
    ...
    builder.button(text=_(key="pay_button"), callback_data="pay_yk_new:0.5:1.0")
